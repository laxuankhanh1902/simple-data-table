  ...
  if (iconOnly) {
    return (
      <div className="flex items-center gap-1">
        <Tooltip title={`Saved Views (${savedViews.length} saved)`}>
          <Button 
            type="text"
            icon={<BookOutlined />} 
            onClick={() => setShowViewsModal(true)}
            className="text-gray-300 hover:text-white hover:bg-gray-800 border border-gray-600 hover:border-blue-500 rounded-lg p-2 relative"
          >
            {savedViews.length > 0 && (
              <span className="absolute -top-1 -right-1 text-xs bg-green-600 text-white px-1.5 py-0.5 rounded-full font-medium min-w-[20px] text-center">
                {savedViews.length}
              </span>
            )}
          </Button>
        ...
        <Tooltip title="Save Current View">
          <Button 
            ...
            className="text-gray-300 hover:text-white hover:bg-gray-800 border border-gray-600 hover:border-blue-500 rounded-lg p-2 disabled:opacity-50 disabled:cursor-not-allowed"
          />
        </Tooltip>
